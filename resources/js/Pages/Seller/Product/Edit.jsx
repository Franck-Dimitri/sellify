...
export default function Edit({ shop, product, remainingStock = 30, maxStockLimit = 30 }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        images: [],
        keep_images: product.image_paths || []
    });
                ...
                <div className="bg-white border border-stone-200/70 rounded-xl p-6 shadow-xs space-y-5">
                    ...
                    <form onSubmit={submit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-xs font-medium text-stone-700 mb-1">Stock Disponible *</label>
                                <input
                                    type="number"
                                    value={data.stock}
                        ...
                        {existingImages.length > 0 && (
                            <div>
                                <label className="block text-xs font-medium text-stone-700 mb-1">Photos Actuelles</label>
                                <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                                    {existingImages.map((path, idx) => (
                                        <div key={idx} className="relative w-full h-16 rounded-lg overflow-hidden border border-stone-200 group">
                                            <img src={`/storage/${path}`} alt="Existe" className="w-full h-full object-cover" />
                                            <button
                                                type="button"
                        ...
                        <div>
                            <label className="block text-xs font-medium text-stone-700 mb-1">Ajouter de Nouvelles Photos</label>
                            ...
                            {imagePreviews.length > 0 && (
                                <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 pt-3">
                                    {imagePreviews.map((src, idx) => (
                                        <div key={idx} className="relative w-full h-16 rounded-lg overflow-hidden border border-stone-200 group">
                                            <img src={src} alt="Nouveau" className="w-full h-full object-cover" />
                                            <button
                                                type="button"
