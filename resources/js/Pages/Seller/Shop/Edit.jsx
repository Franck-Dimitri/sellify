...
export default function Edit({ shop }) {
    ...
    const primaryColor = shop.theme_color || '#CA8A04';
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        theme_color: shop.theme_color || '#CA8A04',
        ...
        <ShopConsoleLayout shop={shop} title={`Configuration - ${shop.name}`}>
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-surface-700 tracking-tight">Configuration de la boutique</h1>
                            ...
                            <div className="flex justify-between items-center">
                                {stepsList.map((s, index) => (
                                    <React.Fragment key={s.id}>
                                        ...
                                        >
                                            <span className={`w-7 h-7 rounded-lg flex items-center justify-center font-medium text-xs transition-all duration-200 
                                                ${step === s.id 
                                                    ? 'text-white shadow-sm ring-2 ring-opacity-20' 
                                                    : step > s.id 
                                            ...
                                            <span className={`text-xs font-medium hidden sm:inline ${step === s.id ? 'text-surface-700 font-semibold' : 'text-surface-450'}`}>
                                                {s.name}
                                            </span>
                                        </button>
                        ...
                        <form onSubmit={submit} className="space-y-6">
                            {step === 1 && (
                                <div className="space-y-5">
                                    <div className="border-b border-surface-100 pb-3">
                                        <h2 className="text-base font-semibold text-surface-700 flex items-center space-x-2">
                                    ...
                                    <div className="flex flex-col space-y-1.5">
                                        <label className="text-sm font-medium text-surface-600">Description</label>
                                        <textarea
                                            className="w-full px-3.5 py-2 text-surface-700 bg-white border border-surface-200 rounded-lg outline-none text-sm min-h-[100px] focus:border-yellow-600 focus:ring-2 focus:ring-yellow-100 transition-all duration-200"
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <label className="text-sm font-medium text-surface-600">Logo de la boutique (1:1)</label>
                                            <div className="border-2 border-dashed border-surface-200 rounded-xl p-4 flex flex-col items-center justify-center hover:border-yellow-500 transition-all bg-surface-50 cursor-pointer relative overflow-hidden group">
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                                ...
                                                />
                                                <Upload className="w-6 h-6 text-surface-400 mb-2 group-hover:text-yellow-600 transition-colors" />
                                                <span className="text-xs font-medium text-surface-600">Remplacer le logo</span>
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-sm font-medium text-surface-600">Bannière (16:9)</label>
                                            <div className="border-2 border-dashed border-surface-200 rounded-xl p-4 flex flex-col items-center justify-center hover:border-yellow-500 transition-all bg-surface-50 cursor-pointer relative overflow-hidden group">
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                                ...
                                                />
                                                <Upload className="w-6 h-6 text-surface-400 mb-2 group-hover:text-yellow-600 transition-colors" />
                                                <span className="text-xs font-medium text-surface-600">Remplacer la bannière</span>
                                    ...
                                    <div className="space-y-2.5">
                                        <label className="text-sm font-medium text-surface-600">Couleur d'accentuation (Style de la page)</label>
                                        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                                            {themePresets.map((preset) => (
                                                <button
                                                    ...
                                                    className={`flex flex-col items-center justify-center p-2 rounded-xl border text-xs font-medium transition-all duration-200
                                                        ${data.theme_color === preset.value 
                                                            ? 'border-yellow-600 bg-surface-50 shadow-sm ring-1 ring-yellow-600' 
                            ...
                            {step === 3 && (
                                <div className="space-y-5">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <Input
                                            label="Lien WhatsApp direct (Numéro format international)"
                                            name="whatsapp"
                                    ...
                                    {/* Opening Hours Grid */}
                                    <div className="space-y-2.5">
                                        <label className="text-sm font-medium text-surface-600 flex items-center space-x-1.5">
                                                ...
                                                <div key={day} className="flex items-center justify-between p-3 bg-white text-sm hover:bg-surface-50 transition-colors">
                                                    <div className="flex items-center space-x-3">
                                                        <input
                                                            ...
                                                            className="w-4.5 h-4.5 rounded border-surface-300 focus:ring-yellow-500"
                                                            style={{ color: shop.theme_color }}
                                                        />
                                                        <label htmlFor={`check-${day}`} className="font-medium text-surface-600 select-none min-w-[70px]">
                                                            {daysTranslation[day]}
                                                        </label>
                                                    </div>
                                                    ...
                                                    {data.opening_hours[day].active ? (
                                                        <div className="flex items-center space-x-2">
                                                             <input
                                                                ...
                                                                className="px-2 py-1 border border-surface-200 rounded-md text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-yellow-500"
                                                            />
                                                        ...
                                                        <span className="text-xs font-medium text-red-500 uppercase pr-4">Fermé</span>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
                ...
                <div className="lg:col-span-5 lg:sticky lg:top-24 h-fit space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-sm font-medium text-surface-400 uppercase tracking-wider">Aperçu en temps réel</h3>
                        <span className="text-[10px] text-surface-400 font-medium flex items-center space-x-1">
                    ...
                    {/* The Mock Store Card */}
                    <div className="bg-white border border-surface-200 rounded-3xl overflow-hidden shadow-sm flex flex-col min-h-[480px]">
                        {/* Banner area */}
                        <div className="h-32 w-full relative bg-surface-850 overflow-hidden flex items-center justify-center">
                            {bannerPreview ? (
                                <img src={bannerPreview} alt="Aperçu Bannière" className="w-full h-full object-cover" />
                            ) : (
                                <div className="absolute inset-0 bg-gradient-to-r opacity-20" style={{ backgroundImage: `linear-gradient(135deg, ${data.theme_color} 0%, #1e293b 100%)` }} />
                            )}
                        ...
                        <div className="px-6 pb-6 relative flex-1 flex flex-col justify-between">
                            ...
                            <div className="flex justify-between items-end -mt-10 mb-4 z-10">
                                ...
                                <Badge variant="success" className="h-6 flex items-center space-x-1 bg-yellow-50 text-yellow-800 border-yellow-250 font-medium">
                            ...
                            <div className="space-y-1.5 flex-1">
                                <h4 className="text-xl font-semibold text-surface-750 tracking-tight">
                                    {data.name || 'Nom de ma boutique'}
                                </h4>
                                <p className="text-sm text-surface-450 italic font-normal">
                            ...
                            <div className="mt-4 pt-3 border-t border-surface-100 space-y-2 text-xs">
                                <div className="flex items-center space-x-2 text-surface-600">
                                    ...
                                    <span className="font-medium">{data.company_name || 'Raison Sociale de l\'entreprise'}</span>
                                </div>
                                {data.registration_number && (
                                    <div className="text-[10px] text-surface-400 pl-6">
                                        RCCM: <span className="font-medium">{data.registration_number}</span>
                                    </div>
                                )}
                                <div className="flex items-center space-x-2 text-surface-600">
                            ...
                            <div className="mt-6 flex space-x-2">
                                <button 
                                    type="button" 
                                    disabled 
                                    className="flex-1 py-2 text-xs font-medium rounded-xl border border-surface-200 bg-white text-surface-600 flex items-center justify-center space-x-1 cursor-default"
                                >
                                ...
                                <button 
                                    type="button" 
                                    disabled 
                                    className="flex-1 py-2 text-xs font-medium rounded-xl text-white flex items-center justify-center space-x-1.5 shadow-sm opacity-90 cursor-default"
