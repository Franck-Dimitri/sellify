...
export default function Customers({ customers, filters, stats }) {
    ...
    return (
        <AdminLayout title="Espace Clients">
            <Head title="Base Clients - Sellify Admin" />
            ...
            <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-yellow-500 to-yellow-400 text-white rounded-3xl p-6 shadow-lg shadow-yellow-500/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                        <h2 className="text-xl font-black tracking-tight flex items-center">
                            <FolderSync className="w-5.5 h-5.5 mr-2" />
                            <span>Espace Clients</span>
                        </h2>
                        <p className="text-xs text-yellow-50 font-bold uppercase tracking-wider">Vue d'ensemble de la base clients</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-sm border border-white/15 px-4.5 py-2 rounded-2xl text-right">
                        <span className="text-sm font-black tracking-tight">{stats.total} Clients / {stats.active} Actifs</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="p-5 rounded-3xl border border-surface-200 bg-white flex items-center justify-between shadow-sm transition-all hover:shadow-md">
                        <div className="space-y-1">
                            <span className="text-xs font-bold text-surface-400 uppercase tracking-wider block">Total Clients</span>
                ...
                {customers.data.length === 0 ? (
                    <Card className="bento-card text-center py-20">
                        <CardContent className="space-y-3">
                            ...
                            <p className="text-sm font-bold text-surface-400">Aucun client trouvé.</p>
                        </CardContent>
                    </Card>
                ) : (
                            ...
                            <div key={user.id} className="bg-white border border-surface-200 rounded-3xl p-5 shadow-sm hover:shadow-md transition-all duration-200 relative flex flex-col justify-between space-y-4">
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center space-x-3.5">
                                        <div className="w-11 h-11 rounded-2xl bg-yellow-50 text-yellow-700 border border-yellow-100 flex items-center justify-center font-black text-sm shadow-sm">
                                            {user.first_name[0]}{user.last_name[0]}
                                        </div>
                                        <div>
                                            <h4 className="font-extrabold text-surface-900 leading-tight">{user.first_name} {user.last_name}</h4>
                                            <span className="text-[10px] text-surface-400 font-bold uppercase tracking-wider block mt-1">Client</span>
                                        </div>
                                    </div>
                ...
                {customers.links.length > 3 && (
                    <Card className="bento-card overflow-hidden p-0">
                        <div className="bg-surface-50 px-6 py-4 flex justify-between items-center text-xs font-bold text-surface-500 font-mono">
                            <div>Affichage de {customers.from} à {customers.to} sur {customers.total} clients</div>
