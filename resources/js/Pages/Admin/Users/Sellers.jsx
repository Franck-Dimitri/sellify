...
export default function Sellers({ sellers, filters, stats }) {
    ...
    return (
        <AdminLayout title="Espace Vendeurs">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-yellow-500 to-yellow-400 text-white rounded-3xl p-6 shadow-lg shadow-yellow-500/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                        <h2 className="text-xl font-black tracking-tight flex items-center">
                            <Store className="w-5.5 h-5.5 mr-2" />
                            <span>Espace Vendeurs</span>
                        </h2>
                        <p className="text-xs text-yellow-50 font-bold uppercase tracking-wider">Vue d'ensemble des boutiques de votre plateforme</p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-sm border border-white/15 px-4.5 py-2 rounded-2xl text-right">
                        <span className="text-sm font-black tracking-tight">{stats.total} Boutiques / {stats.pending} En attente</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="p-5 rounded-3xl border border-surface-200 bg-white flex items-center justify-between shadow-sm transition-all hover:shadow-md">
                        <div className="space-y-1">
                            <span className="text-xs font-bold text-surface-400 uppercase tracking-wider block">Total Vendeurs</span>
                ...
                {sellers.data.length === 0 ? (
                    <Card className="bento-card text-center py-20">
                        <CardContent className="space-y-3">
                            <ShoppingBag className="w-12 h-12 text-surface-300 mx-auto" />
                            <p className="text-sm font-bold text-surface-400">Aucun vendeur trouvé.</p>
                        </CardContent>
                    </Card>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {sellers.data.map((user) => {
                            const sellerStatus = user.seller?.status || 'pending';
                                ...
                                <div key={user.id} className="bg-white border border-surface-200 rounded-3xl p-5 shadow-sm hover:shadow-md transition-all duration-200 relative flex flex-col justify-between space-y-4">
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-center space-x-3.5">
                                            <div className="w-11 h-11 rounded-2xl bg-blue-50 text-blue-700 border border-blue-100 flex items-center justify-center font-black text-sm shadow-sm">
                                                {user.first_name[0]}{user.last_name[0]}
                                            </div>
                                            <div>
                                                <h4 className="font-extrabold text-surface-900 leading-tight">{user.first_name} {user.last_name}</h4>
                                                <span className="text-[10px] text-surface-400 font-bold uppercase tracking-wider block mt-1">Vendeur</span>
                                            </div>
                                        </div>
                                        <Badge variant={
                                            sellerStatus === 'approved' ? 'success' :
                                            sellerStatus === 'pending' ? 'warning' : 'danger'
                                        }>
                                            {sellerStatus === 'approved' ? 'Approuvé' : sellerStatus === 'pending' ? 'En attente' : 'Rejeté'}
                                        </Badge>
                                    </div>
                                    ...
                                    <div className="pt-2 flex items-center space-x-2">
                                        {sellerStatus === 'pending' && pendingKyc ? (
                                            <>
                ...
                {sellers.links.length > 3 && (
                    <Card className="bento-card overflow-hidden p-0">
                        <div className="bg-surface-50 px-6 py-4 flex justify-between items-center text-xs font-bold text-surface-500 font-mono font-mono">
                            <div>Affichage de {sellers.from} à {sellers.to} sur {sellers.total} vendeurs</div>
